...
def test(cfg,
    ...
    for batch_i, (imgs, targets, paths, shapes, pad) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            inf_out, train_out = model(imgs)[1]  # inference and training outputs
            t0 += torch_utils.time_synchronized() - t

            # Compute loss
            if is_training:  # if model has loss hyperparameters
                loss += compute_loss(train_out, targets, model.yolo_part.yolo_detector)[1][:3]  # GIoU, obj, cls
